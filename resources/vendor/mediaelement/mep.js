	...
	$.extend( MediaElementPlayer.prototype, {
		buildmepcurrentdetails: function( player, controls, layers ) {
			...
			player.$node.on( 'setTrack.mep', function( e, track, player ) {
				...
				$title.html( '<a id="track-title" href="'+track.link+'">'+track.title+ '</a>');
			});
		}
	});
...
(function( window, $, undefined ) {
	...
	historySuccess = function( media, domObject, player ) {
		var bookid = $("#track-title").attr('href').split('/');
	...
	$.extend( MediaElementPlayer.prototype, {
		...
		buildmephistory: function( player, controls, layers, media ) {
			...
			media.addEventListener( 'play', function() {
				var bookid = $("#track-title").attr('href').split('/');
			    bookid = bookid[2];
				$.post('/booksmark', {id: bookid, type: 'get'}, function(data){
					currentTime = data;
			...
			media.addEventListener( 'timeupdate', function() {
				...
				if(time != 0){
			    	var fulltime = $('.mejs-duration').html();
			    	fulltime = fulltime.split(':');
			    	fulltime = Number(fulltime[0]*60*60)+Number(fulltime[1]*60)+Number(fulltime[2]);
			    	var bookid = $("#track-title").attr('href').split('/');
			    	bookid = bookid[2];
			    	if(fulltime == time){
						console.log('close');
			    		$.post('/delbooksmark', {id: bookid}, function(data){});
			    	}
			    }   
			});
	...
	function History( id, signature ) {
		...
		this.set = function( key, value ) {
			...
			var bookid = $("#track-title").attr('href').split('/');
			bookid = bookid[2];
            if(key === 'currentTime' && value > 1){
		    $.post('/booksmark', {id: bookid, data: value, type: 'set'}, function(result){});
			}
		};
		...
		this.get = function( key ) {
			...
			if ( 'undefined' !== typeof data[ prop ] ) {
				...
				if ( 'currentTime' === key ) {
					...
					var bookid = $("#track-title").attr('href').split('/');
					$.post('/booksmark', {id: bookid[2], type: 'get'}, function(result){
  ...
  $.extend( MediaElementPlayer.prototype, {
    buildmepbuffering: function(player, controls, layers, media) {
      ...
      media.addEventListener('error',function(e) {
		if($("#track-title").html() != 'Книга не выбрана'){
        $container.removeClass('is-buffering');
        $('body').removeClass('is-buffering');
        error.show();
