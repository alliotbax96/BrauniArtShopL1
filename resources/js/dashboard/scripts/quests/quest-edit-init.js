...
$(document).ready(function () {
        var rowCount = 0; // Счётчик строк

        // Функция для создания новой строки с уникальными индексами
        function createNewRow(index) {
            ...
            <tr id="addr${index}">
                ...
                <td>
                    <input type="number" name="additional_services[${index}][price]"
        ...
        $("#add_row").click(function () {
            // Создаём новую строку с текущим индексом
            var newRow = createNewRow(rowCount);
        ...
        // Удаление строки (делегирование событий для динамически добавляемых кнопок)
        $('#tab_logic tbody').on('click', '.remove-row', function () {
            // Удаляем строку, содержащую нажатую кнопку
            $(this).closest('tr').remove();
            rowCount--;

            // Обновляем нумерацию и индексы во всех оставшихся строках
            updateRowIndices();
        });

        // Функция для обновления нумерации и индексов после удаления
        function updateRowIndices() {
            // Если строк нет, просто выходим
            if (rowCount === 0) {
                return;
            }

            $('#tab_logic tbody tr').each(function (index, row) {
                var $row = $(row);

                // Обновляем номер в первом столбце
                $row.find('td:first-child').text(index + 1);

                // Обновляем ID строки
                $row.attr('id', 'addr' + index);

                // Обновляем атрибуты name у полей ввода
                $row.find('input[name*="[name]"]').attr('name', `additional_services[${index}][name]`);
                $row.find('input[name*="[price]"]').attr('name', `additional_services[${index}][price]`);
            });
        }
});
$(document).ready(function () {
...
$(document).ready(function () {
        $('#quest-edit').on('submit', function (e) {
            e.preventDefault(); // Предотвращаем стандартную отправку формы
            const $form = $(this);
            const submitBtn = $form.find('button[type="submit"]');
            const originalBtnText = submitBtn.text();
            // Блокируем кнопку и показываем индикатор загрузки
            submitBtn.prop('disabled', true).text('Изменение...');
            $.ajax({
                url: '/seller/quests/'+$form.find('input[name="QuestId"]').val(),
                method: 'POST',
                data: $form.serialize(),
                dataType: 'json',
                success: function (response) {
                    ...
                    if (response.success) {
                        ...
                        $('#quest-alert').html('<div class="alert alert-success">Квест успешно изменен</div>');
                        ...
                        handleError(response.message || 'Произошла ошибка при изменении квеста');
                    }
                },
                error: function (xhr) {
                    if (xhr.status === 422) {
                        ...
                        console.log(xhr);
                        handleError(xhr.responseJSON?.message || 'Произошла ошибка при изменении квеста');
